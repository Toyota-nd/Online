...
function setPdf(el,els,category) {
if (debug) alert("setPdf");
	...
	if (els === null) {//單表
		tables = $(el).attr('table');
		titles = $(el).attr('title');
		fields = $(el).attr('field');
		keys = $(el).attr('key');

		var col = $(el).jqGrid('getGridParam','colModel');
		for(var i=2; i<col.length; i++) {
			params += '&' + col[i].name + '=' + col[i].width;
		}
		
	} else { //雙表
		tables = $(el).attr('table') + ',' + $(els).attr('table');

		titles = $(el).attr('title') + ',' + $(els).attr('title');

		fields = $(el).attr('field').split(',');
		for (i in fields) {
			fields[i] = $(el).attr('table') + '.' + fields[i] + ' as ' +
			$(el).attr('table') + '__' + fields[i];
		}
		el_fields = fields.join(',');

		fields = $(els).attr('field').split(',');
		for (i in fields) {
			fields[i] = $(els).attr('table') + '.' + fields[i] + ' as ' +
			$(els).attr('table') + '__' + fields[i];
		}
		els_fields = fields.join(',');
		fields = el_fields + ',' + els_fields;

		keys = $(el).attr('table') + '.' + $(el).attr('key') + ',' +
					$(els).attr('table') + '.' + $(els).attr('key');

		var col = $(el).jqGrid('getGridParam','colModel');
		for(var i=2; i<col.length; i++) {
			params += '&' + $(el).attr('table') + '__' + col[i].name + '=' + col[i].width;
		}

		var col = $(els).jqGrid('getGridParam','colModel');
		for(var i=2; i<col.length; i++) {
			params += '&' + $(els).attr('table') + '__' + col[i].name + '=' + col[i].width;
		}

	}
	var container = 
		'<div width="860px" height="480px"><object id="pdfView" name="pdfPlayer" ' +
		'width="860px" height="480px" type="application/pdf" data="'+
		baseURL + '/index/sql?'+
		'_event=print'+
		'&_sql='+ 'print'+
		'&_db='+ $(el).attr('db')+
	...
	$(container).dialog({
	    ...
	    width: 860,
        modal: true,
		title: '報表產生完成!(需等待大約30秒)'
 	});
	myalert('報表產生完成!(需等待大約30秒)');
	/*
...
//if ($('#exampaper_id').val().length > 0) {
		...
		$.ajax({
		  ...
		  error: function(jqXHR, exception) {
				if (jqXHR.status === 0) {
					alert('Treeview.js:Not connect.\n Verify Network. \n Or, a recursive call is infinite!');
				} else if (jqXHR.status == 404) {
			...
			beforeSend:function(xhr, s) {
			//alert('Treeview.js(function setjqt):'+s.data);
			},	   
		  success: 
			function(data) {
				try {
...
//if ($('#exampaper_id').val().length > 0) {
		...
		$.ajax({
		  ...
		  error: 	function(jqXHR, exception) {
			...
			beforeSend:function(xhr, s) {
			//alert('Treeview.js(function setsjqt):'+s.data);
			},	   
		  success: 
			function(data) {
				try {
...
function getTagAttr(el) {
	if (debug) alert("getTagAttr");
	if (type(el) === "string") {
		el = $("#" + el);
	};
	var str = [];
	str[0] = "_sql="+$(el).attr('sql')+"";
	str[1] = "_type="+$(el).attr('type')+"";
	str[2] = "_db="+$(el).attr('db')+"";
	str[3] = "_table="+$(el).attr('table')+"";
	str[4] = "_field="+$(el).attr('field')+"";
	str[5] = "_key="+$(el).attr('key')+"";
	str[6] = "_value="+$(el).val()+"";
	str[7] = "_pk="+$(el).attr('pk')+"";
	str[8] = "_where="+$(el).attr('where')+"";
	return str.join('&');
}	

function showMoveInfo(position,previous_parent,moved_node,target_node) {
	if (debug) alert("showMoveInfo");	
	switch(position) {
	case 'inside':
		position_name = '把原上級'  + previous_parent.name +
			'所屬的' + moved_node.table+ '的' + moved_node.name 
			+ '，放在' + target_node.table+ '的' +  target_node.name 
			+ '當下級';
		break;
	case 'before':
		position_name = '把原上級' + moved_node.supervisor + 
			'所屬的'  + moved_node.table+ '的' + moved_node.name 
			+ '，放入' + target_node.table+ '的' +  target_node.name
			+ '的前面(同級)';
		break;
	case 'after':
		position_name = '把原上級' + moved_node.supervisor + 
		...
		break;
	case 'none':
		position_name = '不能插入在此位置';
		break;
	default:
		position_name = '有錯誤狀況發生!';
		break;
	};
	myalert(position_name);
}

function setGtw(el) {
//setGtw('gtw');
if (debug) alert('setGtw');
if (typeof el === "string") {
	el = $("#" + el);
}
if ($(el).length == 0) {
...
var table = $("#"+gtw_id).attr('table').split(',');
var key = $("#"+gtw_id).attr('key').split(',');
var field = $("#"+gtw_id).attr('field').split(',');
		
/*
//取得伺服端PHP錯誤訊息，除錯專用
	$.ajax({
	  //一定要在PHP宣告且指定正確的baseURL		
	  url: baseURL + "/index/simpletreeview?"+getTagAttr(gtw_id), 
	  cache: false, 
	  dataType: 'text', // '原來為json->改成html,由本函數success:自行檢查'            
	  type:'POST',
	  error: 	function(jqXHR, exception) {
		...
		beforeSend:function(xhr, s) {
		//alert('listuser.pthml:'+getTagAttr(gtw_id));
		},	   
	  success: 
		function(data) {
			alert(data);
...
$(el).tree({
	dataUrl: baseURL + "/index/simpletreeview?"+getTagAttr(gtw_id),
	//dataUrl: 'http://www.localhost.com/cc/test.php',
	dragAndDrop: true,
	autoOpen: 0,
	selectable: true,
	useContextMenu: true,
...
//		slide: true,
	onCanMoveTo:  function(moved_node, target_node, position) {
		var previous_parent = moved_node.parent;
		showMoveInfo(position,previous_parent,moved_node,target_node);
	...
	function(event) {		
		/*
		$('#'+gtw_id).attr('table',table[0]);
		$('#'+gtw_id).attr('field',field[0]);
		$('#'+gtw_id).val(event.move_info.target_node.id);
		$('#'+gtw_id).attr('key',key[0]);
		$('#'+gtw_id).attr('pk',event.move_info.moved_node.id);
		$('#'+gtw_id).attr('sql','update').trigger('change');
		*/
		//event.move_info.previous_parent.name
		/* 
		update(mydb,mytable,myfield,myvalue,mykey,mypk);
		// 主表記錄階層supervisor異動
		update(('#'+gtw_id).attr('db'),
				event.move_info.target_node.table,
				'supervisor',event.move_info.target_node.id,
				('#'+gtw_id).attr('key'),
				event.move_info.moved_node.id);
		// 明細表記錄FK異動與主表記錄之間的從屬關係
		單表:update table user
			set user.supervisor = event.move_info.target_node.id
			where user.user_id = event.move_info.moved_node.id;
			
			update table tables[i] 
			set tables[i].fields[0] = event.move_info.target_node.id
			where tables[i].key[0] = event.move_info.moved_node.id;				
			
		雙表:update table user join department 
			on department.department_id = user.department_id
			set user.department_id = event.move_info.target_node.id
			where user.user_id = event.move_info.moved_node.id;
			
			update table tables[1] join tables[0]
			on tables[0].key[0] = tables[1].key[0]
			set tables[1].key[0] = "'"+event.move_info.target_node.id+"'"
			where tables[1].key[0] = event.move_info.moved_node.id;
		*/		
		//判斷單表或雙表:
	event.preventDefault();			
	if (confirm('確定要變更嗎?')) {
		var mytables = $('#'+gtw_id).attr('table').split(',');
		var myfields = $('#'+gtw_id).attr('field').split(',');
		if ( event.move_info.target_node.table === 
			 event.move_info.moved_node.table) { //單表
			if ($.inArray( event.move_info.moved_node.table, mytables)==0) { //主表
				update($('#'+gtw_id).attr('db'),
				event.move_info.moved_node.table,
				myfields[0],event.move_info.target_node.id,
				$('#'+gtw_id).attr('key'), //當單表時，用來當PK查詢定位
				event.move_info.moved_node.id);
			} else {
				//printObject(event.move_info.target_node.parent.children[0]);
				update($('#'+gtw_id).attr('db'),
				$('#'+gtw_id).attr('table'),
				'', //(單表有用到改supervisor，但雙表沒用到，雙表預設為關聯規則之變更
				event.move_info.target_node.parent.children[0].id,
				$('#'+gtw_id).attr('key'), //關聯規則之變更：當雙表時，用來當FK被變更指向新PK
				event.move_info.moved_node.id);
			}
		} else { //雙表，固定找出明細表的PK，改其FK指向主表新的PK
				update($('#'+gtw_id).attr('db'),
				$('#'+gtw_id).attr('table'),
				'', //(單表有用到改supervisor，但雙表沒用到，雙表預設為關聯規則之變更
		...
		tag_name = "原上級:" +
					event.move_info.previous_parent.name +
				   "db=" + $('#'+gtw_id).attr('db') + "," + 
				   "table=" + $('#'+gtw_id).attr('table') + "," + 
				   "field=" + $('#'+gtw_id).attr('field') + "," + 
				   "new value=" +event.move_info.target_node.id + "," + 
				   "moved.table=" +event.move_info.moved_node.table + "," + 
				   "target.table=" +event.move_info.target_node.table + "," + 
				   "pk=" + event.move_info.moved_node.id + " " + 
				   "sql=" + $('#'+gtw_id).attr('sql') + " ";
...
//	$(window).click(function () {$('#progress').show();}); //$('#div_tags').show();
	 ...
	 */
	$('#backlogo').on('dblclick', function() {
		$('#browsefile').click(function(event) {
			printObject(handleObj);
			//document.forms["browsefileform"].submit();
		}
		);
	});
