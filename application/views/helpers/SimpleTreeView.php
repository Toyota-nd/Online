<?php
class Zend_View_Helper_SimpleTreeView extends Zend_View_Helper_Abstract {
protected $db;
...
public function simpleTreeView( $no, $index, $cnt, $type, $sql, $db, $table, $field, $key, $id, $value, $where, $totag) {

	if ($type == 'gtw') {
		$tree = $this->gtw(array(), $index, $db, $table, $field, $key, $id, $value, $where);
		return json_encode($tree);
	} else {
		$tree = $this->stw($no, $index, $cnt, $type, $sql, $db, $table, $field, $key, $id, $value, $where, $totag);
		return $tree;
	}
}

public function gtw($tree, $index, $db, $table, $field, $key, $id, $value, $where) {
	...
	$sql_command = "select * from $db." . $tables[0] .
		" where " . $fields[0] . " ='$index' order by " . $fields[0];
	...
	while($arr = $q->fetch()){
        ...
        if(count($subFileCount->fetchAll()) > 0) {
			...
			$tree[] = array(
				"id" => $id, 
				"label" => $id . " " .$arr[$fields[1]], 
				"table" => $tables[0], 
				"supervisor" => $arr[$fields[0]], 
				"children" => $this->gtw($tree["children"],$id,
					$db, $table, $field, $key, $id, $value, $where)
			);
        } else {
			$tree[] = array("id" => $id, "label" => $id . 
				" " . $arr[$fields[1]],
				"table" => $tables[0],
				"supervisor" => $arr[$fields[0]]
				);
		...
		if (count($tables) > 1) {
			...
			while ($rows = $result->fetch()) {
				$tree[] = array(
					"id" => $rows[$tables[1] . '_id'], 
					"label" => $rows[$tables[1] . '_id'] . ' ' . $rows[$fields[1]],
					"table" => $tables[1],
					"supervisor" => $arr[$fields[0]]
				);		
			}
...
function leaf_helper ($db, $tables, $fields, $key, $id) {
	$str .= '';
	$result = $this->db->query("select * from $db." . $tables[0] . ' join ' . "$db." . $tables[1] . 
				' on ' . $tables[0] . '.' . $tables[0] . '_id = ' . $tables[1] . '.' . $tables[0] . 
				'_id where ' . $tables[0] . '.' . $key . ' = ' . "'$id'");
	while ($rows = $result->fetch()) {
		$str .= $rows[$tables[1] . '_id'] . ' ' . $rows[$fields[0]] . '<br/>';
	}
	return $str;
}

public function stw( $no, $index, $cnt, $type, $sql, $db, $table, $field, $key, $id, $value, $where, $totag) {
	...
	if ($where === null) {
		$sql_command = "select * from " . $tables[0] ." where supervisor ='$index' order by supervisor";
	} else {
		$sql_command = "select * from " . $tables[0] ." where supervisor ='$index' order by supervisor";
	}
	...
	{
		$id = $arr[$key];
		$subFileCount=$this->db->query("select * from " . $tables[0] ." where supervisor = '$id' order by supervisor");
		if ($flag > 0) {
			$tree .= ','; 
		}
		if(count($subFileCount->fetchAll()) > 0) {
			...
			//'file'
			$label = $no+1;
			//$expain = '"' . $arr["name"] . ':' . $arr[$tables[0].'_id'] .'":"'. 
			//		$this->ui_helper ($type, $sql, $db, $tables, $fields, $key, $id, $no+1, $arr['url'], $arr["name"], $totag) 
			//		. '"';
			$no++;
		}
		if ($index > 0) {
		...
		$tree .= '"' . $this->ui_helper ($type, $sql, $db, $tables, $fields, $key, $id, $no+1, $arr['url'], $arr[$fields[0]],$checked,$myvalue, $totag) . '",'; //$label
		$tree .= '"items" : {';
		$tree .= $expain; 
		//print_r("[" . $flag . "]" .  $arr[$fields[0]]);
		$flag = 0;
		$tree .= $this->stw($no, "".$arr[$key]."",$flag, $type, $sql, $db, $table, $field, $key, $id, $value, $where, $totag);
		$flag = $flag + 1;
		$tree .=  "}";
		$tree .= '}';
	}
...
function ui_helper ($type, $sql, $db, $tables, $fields, $key, $id, $label, $url ,$value, $checked, $myvalue, $totag) {
		//$str = "<table border='0'>";
		/*
		...
		case "select":
			switch ($type) {
				case "auto":
					...
					switch ($rows["mytype"]) {
						...
						case "checkbox":
							...
							$str .= $this->marker($db,$tables,$key,$id);
							$sql_command =  
							"select * from $db." . $tables[0] . ' join ' . "$db." . $tables[1] . 
										' on ' . $tables[0] . '.' . $tables[0] . '_id = ' . $tables[1] . '.' . $tables[0] . 
										'_id where ' . $tables[0] . '.' . $key . ' = ' . "'$id'";
				...
				case "checkbox":
					...
					//$str .= $this->marker($db,$tables,$key,$id);
					$sql_command = "select * from $db." . $tables[0] . ' join ' . "$db." . $tables[1] . 
								' on ' . $tables[0] . '.' . $tables[0] . '_id = ' . $tables[1] . '.' . $tables[0] . 
								'_id where ' . $tables[0] . '.' . $key . ' = ' . "'$id'";
					$result = $this->db->query($sql_command);
					//Master=> 0:MasterSelect 1:MasterUpdate
					//MasterDetail=> 0:MasterSelect 1:MasterUpdate 2:DetailSelect 3:DetailUpdate					
					$ms_pos = 0;
					$mu_pos = 1;
					$ds_pos = 2;
					$du_pos = 3;
//					$str = "<input id='selectall' type='checkbox'/>" . $id . ":" . $value . '<br/>';
					$str .= "<div>";					
					$str .= "			<input id='checkbox_$id' onchange='setSelectall(this);' group= 'selectall' type='checkbox' " . $checked . " field='" . $fields[$mu_pos] . 
							"' value='" . $myvalue . "'"; 
					$str .= "			class='database' sql='update' db='".$db."' table='" . 
												$tables[0] . "' key='" . $tables[0] . "_id' pk='" . $id . "'/>";
					$str .= "			" . $id . ":" . $value . '<br/>'; //0

					while ($rows = $result->fetch()) {
						$label0 = $rows[$tables[0]."_id"];  // Different
						$label1 = $rows[$tables[1]."_id"];  // Different
						if ($rows[$fields[$du_pos]]) { 
							$checked = "checked";
							$myvalue = "1";						
						} else {
							$checked = "unchecked";
						...
						$str .= "		<label for='" . "checkbox_". $label0 . "_" . $label1 . "'>";
						$str .= "			<input type='checkbox' " . $checked . " onchange=setRelation(this,'$totag');" . " field='" . $fields[$du_pos] . "' name='checkbox".  $id . "[]" . 
								"' id='" ."checkbox_". $label0 . "_" . $label1 . "' value='" . $myvalue . "'"; 
						$str .= "			class='database' sql='update' db='".$db."' table='" . 
													$tables[1] . "' key='" . $tables[1] . "_id' pk='" . $label1 . "'/>";
						$str .= "			" . $rows[$fields[$ms_pos]]; //0
						$str .= "		</label>";
						$str .= "		<br/>";
						if (isset($url)) {
							$str .= "	<img id='im" . $imageid . "' class='myimage_class' width='300' height='200' title='按一下圖放大，按兩下圖縮小' src='" . $url . "'>";	
